#!/bin/bash
#
# Uamgo CLI installer — https://uamgo.com/cli/install.sh
#
# Auth: UAMGO_DEPLOYMENT_KEY (takes precedence) or ~/.uamgo/auth.json from `uamgo login`.
# Env: UAMGO_CHANNEL (stable|alpha|enterprise, default: stable), UAMGO_BIN_DIR, UAMGO_PROXY_URL
#
# Usage:
#   curl -fsSL https://uamgo.com/cli/install.sh | bash            # latest stable
#   curl -fsSL https://uamgo.com/cli/install.sh | bash -s 0.1.42  # specific version
#   UAMGO_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://uamgo.com/cli/install.sh)
#
...
# Read a token from ~/.uamgo/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_uamgo_token() {
    local auth_file="$HOME/.uamgo/auth.json"
    local scope="$1"
...
# Resolve auth: UAMGO_DEPLOYMENT_KEY > OIDC token > legacy token
OIDC_SCOPE="https://auth.uamgo.com::b1a00492-073a-47ea-816f-4c329264a828"
LEGACY_SCOPE="https://accounts.uamgo.com/sign-in"
AUTH_SOURCE=""

if [ -n "$UAMGO_DEPLOYMENT_KEY" ]; then
    AUTH_SOURCE="deployment key"
    echo "Auth: using deployment key." >&2
else
    OIDC_TOKEN=$(read_uamgo_token "$OIDC_SCOPE" 2>/dev/null) || true
    LEGACY_TOKEN=$(read_uamgo_token "$LEGACY_SCOPE" 2>/dev/null) || true
    if [ -n "$OIDC_TOKEN" ]; then
        AUTH_SOURCE="auth.json (oidc)"
        echo "Auth: using OIDC token from ~/.uamgo/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.uamgo/auth.json." >&2
    fi
fi
...
BASE_URL="${UAMGO_DIST_BASE_URL:-https://uamgo.com/cli}"
DOWNLOAD_DIR="$HOME/.uamgo/downloads"
BIN_DIR="${UAMGO_BIN_DIR:-$HOME/.uamgo/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${UAMGO_CHANNEL:-stable}"
case "$CHANNEL" in
    stable|alpha|enterprise) ;;
    *)
        echo "Invalid UAMGO_CHANNEL: '${CHANNEL}' (expected stable, alpha, or enterprise)" >&2
        exit 1
        ;;
esac

# The channel pointer doubles as the reachability probe, so the happy path
# costs no extra request. $UAMGO_DIST_BASE_URL retargets a mirror.
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Uamgo $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Uamgo $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/uamgo-$platform"
artifact_base="${BASE_URL}/uamgo-${version}-${platform}"
...
echo "  Downloading uamgo ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! fetch_binary "${artifact_base}.exe" "$binary_tmp"; then
        if ! fetch_binary "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Uamgo is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in uamgo.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/uamgo.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded uamgo failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.uamgo/bin and ~/.uamgo/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/uamgo"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/uamgo and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.uamgo/completions/bash" "$HOME/.uamgo/completions/zsh"
"$BIN_DIR/uamgo" completions bash > "$HOME/.uamgo/completions/bash/uamgo.bash" 2>/dev/null || true
"$BIN_DIR/uamgo" completions zsh  > "$HOME/.uamgo/completions/zsh/_uamgo"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/uamgo" completions fish > "$HOME/.config/fish/completions/uamgo.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.uamgo/config.toml"
...
if [ -n "$UAMGO_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${UAMGO_PROXY_URL:-https://cli-chat-proxy.uamgo.com/v1}"
    ...
    if [ -z "$proxy_ok" ]; then
        echo "Error: UAMGO_PROXY_URL must be an https:// URL." >&2
        exit 1
    fi
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$UAMGO_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.uamgo/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.uamgo/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.uamgo/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.uamgo/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Uamgo $version installed to $BIN_DIR/uamgo.exe" >&2
else
    echo "Uamgo $version installed to $BIN_DIR/uamgo" >&2
fi

# --- Ensure uamgo is on PATH ---

path_has_dir() {
    case ":$PATH:" in *":$1:"*) return 0 ;; *) return 1 ;; esac
}

# Try to symlink into a directory already on PATH so uamgo works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/uamgo" "$candidate/uamgo"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/uamgo -> $BIN_DIR/uamgo" >&2
...
# Also update shell config so ~/.uamgo/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> uamgo installer >>>
fish_add_path $HOME/.uamgo/bin
# <<< uamgo installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> uamgo installer >>>
export PATH="$HOME/.uamgo/bin:$PATH"
fpath=(~/.uamgo/completions/zsh $fpath)
autoload -Uz compinit && compinit -C
# <<< uamgo installer <<<'
    else
        new_block='# >>> uamgo installer >>>
export PATH="$HOME/.uamgo/bin:$PATH"
[[ -r "$HOME/.uamgo/completions/bash/uamgo.bash" ]] && source "$HOME/.uamgo/completions/bash/uamgo.bash"
# <<< uamgo installer <<<'
    fi

    if grep -qs "uamgo installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> uamgo installer >>>/ { skip=1; next }
            /# <<< uamgo installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'uamgo' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'uamgo' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'uamgo' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.uamgo/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use uamgo from cmd.exe or PowerShell, add %USERPROFILE%\\.uamgo\\bin to your PATH." >&2
fi
