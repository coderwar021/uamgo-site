#
# Uamgo CLI installer for PowerShell - https://uamgo.com/cli/install.ps1
#
# Auth: UAMGO_DEPLOYMENT_KEY env var (takes precedence) or ~/.uamgo/auth.json from `uamgo login`.
# Env: UAMGO_CHANNEL (stable|alpha|enterprise, default: stable), UAMGO_BIN_DIR, UAMGO_PROXY_URL
#
# Usage:
#   irm https://uamgo.com/cli/install.ps1 | iex                                       # latest stable
#   & ([scriptblock]::Create((irm https://uamgo.com/cli/install.ps1))) -Version 0.1.42 # specific version
#   $env:UAMGO_VERSION="0.1.42"; irm https://uamgo.com/cli/install.ps1 | iex           # specific version (alt)
#   $env:UAMGO_DEPLOYMENT_KEY="<key>"; irm https://uamgo.com/cli/install.ps1 | iex
#
...
if (-not $Version -and $env:UAMGO_VERSION) {
    $Version = $env:UAMGO_VERSION
}

# This script is Windows-only. PS 5.1 has no Platform property and only runs on Windows.
if ($PSVersionTable.Platform -and $PSVersionTable.Platform -ne 'Win32NT') {
    Write-Error "This installer is for Windows. On macOS/Linux, use: curl -fsSL https://uamgo.com/cli/install.sh | bash"
    exit 1
}

$UamgoDir = Join-Path $env:USERPROFILE '.uamgo'
...
function Test-MinGitUsable([string]$VersionDir) {
    # Same predicate as uamgo_tty_utils::bundled_git's is_usable(): the launcher plus the
...
function Install-WindowsPayload([string]$BaseUrl, [string]$Version, [string]$Platform, [string]$BinDir, [string]$DownloadDir) {
    ...
    # siblings of uamgo.exe; uamgo resolves the bundled git from
    # %LOCALAPPDATA%\uamgo\git\<mingit-version>\ (newest usable version wins, so older
    ...
    # Same shape as uamgo-update's windows_payload (which cannot run before uamgo.exe
    ...
    $gitRoot = Join-Path (Join-Path $env:LOCALAPPDATA 'uamgo') 'git'
    $mingitBase = "$BaseUrl/uamgo-$Version-$Platform-mingit"
    ...
    $zipPath = Join-Path $DownloadDir "uamgo-$Platform-mingit.zip"
...
function Read-UamgoToken([string]$Scope) {
    $authFile = Join-Path $UamgoDir 'auth.json'
...
$OidcScope = 'https://auth.uamgo.com::b1a00492-073a-47ea-816f-4c329264a828'
$LegacyScope = 'https://accounts.uamgo.com/sign-in'
$AuthSource = ''

if ($env:UAMGO_DEPLOYMENT_KEY) {
    $AuthSource = 'deployment key'
    Write-Host 'Auth: using deployment key.' -ForegroundColor DarkGray
} else {
    $oidcToken = Read-UamgoToken $OidcScope
    $legacyToken = Read-UamgoToken $LegacyScope
    if ($oidcToken) {
        $AuthSource = 'auth.json (oidc)'
        Write-Host 'Auth: using OIDC token from ~/.uamgo/auth.json.' -ForegroundColor DarkGray
    } elseif ($legacyToken) {
        $AuthSource = 'auth.json (legacy)'
        Write-Host 'Auth: using legacy token from ~/.uamgo/auth.json.' -ForegroundColor DarkGray
    }
}
...
$BaseUrlPrimary = 'https://uamgo.com/cli'
$BaseUrlFallback = 'https://storage.googleapis.com/uamgo-code-public-artifacts/cli'
$DownloadDir = Join-Path $UamgoDir 'downloads'
$BinDir = if ($env:UAMGO_BIN_DIR) { $env:UAMGO_BIN_DIR } else { Join-Path $UamgoDir 'bin' }

New-Item -ItemType Directory -Path $DownloadDir -Force | Out-Null
New-Item -ItemType Directory -Path $BinDir -Force | Out-Null

$Channel = if ($env:UAMGO_CHANNEL) { $env:UAMGO_CHANNEL } else { 'stable' }
if ($Channel -cnotmatch '^(stable|alpha|enterprise)$') {
    Write-Error "Invalid UAMGO_CHANNEL: '$Channel' (expected stable, alpha, or enterprise)"
    exit 1
}

# Pick a working BaseUrl: try Cloudflare-fronted uamgo.com first, fall back to
...
if ($AuthSource) {
    Write-Host "Installing Uamgo $resolvedVersion ($platform, $AuthSource)..." -ForegroundColor Cyan
} else {
    Write-Host "Installing Uamgo $resolvedVersion ($platform)..." -ForegroundColor Cyan
}

# --- Download binary ---

$binaryPath = Join-Path $DownloadDir "uamgo-$platform.exe"
$artifactBase = "$BaseUrl/uamgo-$resolvedVersion-$platform"
...
foreach ($binName in @('uamgo.exe', 'agent.exe')) {
    try {
...
Write-Host "  Installed to $BinDir\uamgo.exe and $BinDir\agent.exe." -ForegroundColor DarkGray

# --- Windows payload (best-effort): grove hook exes beside uamgo.exe + bundled MinGit ---

Install-WindowsPayload $BaseUrl $resolvedVersion $platform $BinDir $DownloadDir

# --- Generate completions (best-effort) ---

$completionsDir = Join-Path (Join-Path $UamgoDir 'completions') 'powershell'
try {
    New-Item -ItemType Directory -Path $completionsDir -Force | Out-Null
    & (Join-Path $BinDir 'uamgo.exe') completions powershell 2>$null |
        Set-Content (Join-Path $completionsDir 'uamgo.ps1') -ErrorAction SilentlyContinue
} catch {}

# --- Persist installer config ---

$ConfigFile = Join-Path $UamgoDir 'config.toml'
...
if ($env:UAMGO_DEPLOYMENT_KEY) {
    $ProxyUrl = if ($env:UAMGO_PROXY_URL) { $env:UAMGO_PROXY_URL } else { 'https://cli-chat-proxy.uamgo.com/v1' }
    # Refuse cleartext / userinfo / empty-host proxies before attaching the key.
    try {
        $proxyUri = [Uri]$ProxyUrl
    } catch {
        Write-Error "UAMGO_PROXY_URL must be an https:// URL."
        exit 1
    }
    ...
    try {
        $headers = @{ 'Authorization' = "Bearer $($env:UAMGO_DEPLOYMENT_KEY)" }
    ...
    if ($deployResponse) {
        ...
        $managedConfigPath = Join-Path $UamgoDir 'managed_config.toml'
        $requirementsPath = Join-Path $UamgoDir 'requirements.toml'
...
Write-Host "Uamgo $resolvedVersion installed to $BinDir\uamgo.exe" -ForegroundColor Green
...
if ($pathEntries -notcontains $BinDir) {
    ...
    # Update current session so uamgo works immediately.
...
Write-Host ''
Write-Host "Run 'uamgo' or 'agent' to get started!" -ForegroundColor Cyan
